...
const fluidPlayerClass = function() {
	...
	self.init = (playerTarget, options) => {
		...
		playerNode.pause = function() {
			...
			// Extra check to ensure video is playing before attempting to pause
			if (self.isCurrentlyPlayingVideo(self.domRef.player)) {
				try {
					self.isPlayingMedia = false;
					return videoPauseOriginal.apply(this, arguments);
				} catch (e) {
					self.announceLocalError(203, 'Failed to pause video.');
				}
			}
		};

		// Autoplay handling with Safari exception
		...
		if (!self.mobileInfo.userOs) {
			...
			// On mobile, show controls on touch start and hide them otherwise
			self.hideControlBar();
		...
		// Keyboard controls and auto-hide for control bar
